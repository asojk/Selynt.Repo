...
export default function SlidePricing() {
	...
	return (
		...
		<div className={theme === 'dark' ? 'invert-colors' : ''}>
			<section className='relative w-full overflow-hidden bg-white px-4 py-12 text-black lg:px-8 lg:py-24 dark:invert dark:prose-invert'>
...
const DESELECTED_STYLES =
	'font-medium rounded-lg py-3 w-28 hover:bg-n-2  transition-colors relative'
...
const Heading = ({selected, setSelected}: HeadingProps) => {
	return (
		<div className='relative z-10 mb-12 lg:mb-24'>
			...
			<h3 className='text-n-900 prose-blockquote:'> basic packages which come with varying but very limiting limitations,
			-annual estimates include monthly rates, cheap themes or add-ons which are necessary in many cases, the cost of your time/knowledge/labor on website maintenance and configuration which we have estimated at an average of $250/month, and domain cost (avg. $20/year).</h3>
		</div>
	)
}

const BackgroundShift = () => (
	<motion.span layoutId='bg-shift' className='absolute inset-0 -z-10 rounded-lg bg-p' />
)
			...
			<div>
				...
				<div className='mb-2 flex w-[90%] items-center gap-2 text-left'>
					<span className='text-sm italic'>
						Additional anytime maintenance or updates billed at $50/hr
					</span>
				</div>
			</div>
			<motion.button
