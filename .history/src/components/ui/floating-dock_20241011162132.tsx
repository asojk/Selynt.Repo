	...
	return (
		<>
			...
			<motion.div
				...
				<div
					className={cn(
						'fixed bottom-0 right-0 z-[1000] flex items-end justify-end transition-all duration-300 rounded-full',
						isOpen ? 'h-[min(100vw,800px)] w-[min(100vw,800px)]' : 'h-20 w-20',
						'origin-bottom-right rounded-full',
						isOpen && 'border-[6rem] border-b-0 border-r-0 border-solid rounded-full border-white',
						className
					)}
					style={{
						scale: scale.get(),
						borderImage: isOpen
							? 'linear-gradient(to bottom right, white, white, transparent, transparent)'
							: 'none',
