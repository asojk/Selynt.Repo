  ...
  return (
    <>
      ...
      <motion.div
        ...
        initial="closed"
        className={cn(
          "fixed bottom-0 left-0 p-4 z-[1000] mx-auto flex items-center justify-center",
          className
        )}
        style={{
          scale,
...
function IconContainer({ mouseX, title, icon, href, index, totalItems }: IconContainerProps) {
  ...
  const angle = (index / (totalItems - 1)) * (Math.PI / 2); // Distribute icons over 90 degrees
  const radius = 80; // Adjust based on your desired size
...
<motion.div
  ...
  className='relative flex items-center justify-center rounded-xl bg-white dark:bg-g-b'
>
