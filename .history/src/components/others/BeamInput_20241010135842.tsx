...
const BeamInput = () => {
  ...
  return (
    <form
      ...
      className="relative flex w-full max-w-md items-center gap-2 rounded-full border-2 border-black/10 dark:border-white/10 bg-gradient-to-br from-p to-p-dark py-1.5 pl-6 pr-1.5"
    >
      <input
        ref={inputRef}
        type="email"
      ...
      <button
        ...
        type="submit"
        className="group flex shrink-0 items-center hover:gap-1.5 rounded-full bg-gradient-to-br from-gray-50 to-gray-400 px-4 py-3 text-sm font-medium text-gray-900 transition-transform active:scale-[0.985]"
      >
      ...
      <div className="pointer-events-none absolute inset-0 z-10 rounded-full">
        <motion.div
          ...
          className="mask-with-browser-support absolute -inset-[1px] rounded-full border border-transparent bg-origin bg-white/10 bg-blend-overlay backdrop-blur-xl"
        />
      </div>
    </form>
  );
